...
Rate limits are per user and use a rolling 60 second window. When a limit is exceeded, the API returns HTTP 429 with details about the triggered window, current value, and limit.

- `/v2/search`, `/v2/search/submit`, and `/v2/search/count`: 60 actual search/count requests per 60 seconds, shared across these endpoints. Fully cached `/v2/search` thread reloads that do not run a new search use a separate 120 requests per 60 seconds limit.
- `/v2/company`: 180 requests per 60 seconds.
- `/v1/search`: 60 requests per 60 seconds.
- `/v1/profile`: 60 requests per 60 seconds.
- `/v1/profile/batch`: 180 batch requests per 60 seconds. Each batch counts as one request regardless of item count.
...
Set `time_budget` to customize the target search time in seconds. When omitted, it defaults to 30 seconds for `type="fast"` and 200 seconds for `type="pro"`:
...
## Count matching profiles (/v2/search/count)

Use `search_count` to count profiles without returning profile records. It accepts a natural-language `query`, structured `search_requirements`, or `custom_filters`. Filters can be combined with either natural-language input.

Count pricing depends on the request:

| Request | Without stats | With `stats=True` |
| --- | ---: | ---: |
| Custom filters only | 2 credits | 7 credits |
| `query` or `search_requirements` (with or without custom filters) | 5 credits | 10 credits |

`stats=True` adds 5 credits and returns field statistics calculated from a sample of up to 1,000 matching profiles. The full matching count remains available as `response.count` and `response.stats.matched_profiles`.

```python
from pearch import CustomFilters, PearchClient, V2SearchCountRequest

with PearchClient() as client:
    response = client.search_count(V2SearchCountRequest(
        query="Python engineers in Germany",
        custom_filters=CustomFilters(min_total_experience_years=5),
        stats=True,
    ))

print(response.count)
print(response.credits_used)  # 10

if response.stats:
    skills = response.stats.fields["canonical_skills"]
    print(skills.coverage_pct)
    for item in skills.top_values or []:
        print(item.value, item.profile_count, item.percentage)
```

The stats response includes typed coverage, top-value, numeric distribution, bucket, and boolean-count data. Each repeated categorical value is counted at most once per profile.
...
with PearchClient() as client:
    ...
    filters = CustomFilters(
        ...
        location_coordinates=[
            {"lat": 37.7749, "lng": -122.4194, "radius_miles": 25}
        ],
...
- **Array filters (include)**: `locations`, `location_coordinates`, `languages`, `titles`, `industries`, `companies`, `universities`, `keywords`, `degrees`, `specialization_categories`. Each `location_coordinates` item contains `lat`, `lng`, and a positive `radius_miles`; multiple radius areas are combined with OR.
...
- `docid`: LinkedIn profile ID
- `email`: Email address resolved to a LinkedIn profile
- `github_id`: GitHub handle or profile URL used for exact GitHub analysis
- `high_freshness` (optional): Enable real-time profile updates (`realtime_profiles_cost`)
- `github_enrich` (optional): Add GitHub analysis for the account mapped to the LinkedIn profile (`github_enrich_cost` when analysis is returned)
- `reveal_emails` (optional): Include email addresses if available (`email_cost` when emails are found). Deprecated: `show_emails` (still supported but deprecated)
- `reveal_phones` (optional): Include phone numbers if available (`phone_cost` when phone numbers are found). Deprecated: `show_phone_numbers` (still supported but deprecated)
- `with_profile` (optional): Include full profile data in response (`profile_enrichment_cost`)

Use `GET /v1/user` to retrieve the effective credit value for each pricing key.

Exactly one identifier is normally sufficient. For backward compatibility, `docid` may be supplied with `email`; `docid` takes precedence.
...
### Search GitHub developers (/v2/github_search)

```python
from pearch import PearchClient, V2GithubSearchRequest

with PearchClient() as client:
    response = client.github_search(V2GithubSearchRequest(
        query="React and Node.js backend engineer",
        limit=10,
    ))
```

Results preserve raw `skills` and `oss_contributions`, add the canonical aliases `github_skills` and `github_oss_contributions`, and include the deterministic `github_skills_curated` ranking.

### Enrich profiles in a batch (/v1/profile/batch)

Batch enrichment accepts between 1 and 100 profile identifiers. Each item must contain exactly one `docid`, `uuid`, or `email`. Options apply to the complete batch, results preserve input order, and missing profiles are returned as item-level errors without failing successful items.
...
with PearchClient() as client:
    response = client.enrich_profiles(V1ProfileBatchRequest(
        profiles=[
            V1ProfileBatchItem(
...
for result in response.results:
    if result.http == 200:
        print(result.reference_id, result.profile.linkedin_slug)
    else:
        print(result.reference_id, result.error.status)

print(response.credits_used)
```

The complete batch counts as one rate-limited request regardless of item count. Costs are calculated per successful profile using the same pricing as `/v1/profile`; UUID items use masked enrichment pricing.

### Enrich company (/v2/company)

Retrieve the complete stored company document by LinkedIn company slug or website domain. Pass exactly one identifier. The endpoint does not run realtime company lookup or contact enrichment.

```bash
curl --get 'https://api.pearch.ai/v2/company' \
  --header "Authorization: Bearer ${PEARCH_API_KEY}" \
  --data-urlencode 'linkedin_slug=openai'
```

To look up the company by domain instead:

```bash
curl --get 'https://api.pearch.ai/v2/company' \
  --header "Authorization: Bearer ${PEARCH_API_KEY}" \
  --data-urlencode 'domain=https://www.openai.com/about'
```

URLs are normalized to a lowercase bare domain, so the second request looks up `openai.com`.

The response contains:

- `company`: the stored company document, including identifiers, description, locations, industries, employee counts, funding information, and other available metadata
- `credits_remaining`: the balance after the request
- `credits_used`: the credits charged for this request
- `credits_breakdown`: an itemized charge, or `null` if no credits were charged

Company enrichment costs `company_enrichment_cost` (1 credit by default). Use `GET /v1/user` to retrieve the effective cost for your account.
